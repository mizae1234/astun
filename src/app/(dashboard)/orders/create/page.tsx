...
export default function POSPage() {
  ...
  const handlePhoneChange = async (phone: string) => {
    ...
    setCustomerFound(false);
    if (phone.length >= 9 && selectedCompany) {
      setSearching(true);
      try {
        const customer = await searchCustomerByPhone(phone, selectedCompany);
        if (customer) {
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="relative">
                    <input type="tel" placeholder="เบอร์โทรลูกค้า (ใส่เบอร์เพื่อค้นหาข้อมูลเดิม)" value={customerPhone}
                    ...
                    {customerFound && (
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1 text-xs text-emerald-600">
                        <UserCheck className="w-3.5 h-3.5" /> พบลูกค้าเดิม
                      </span>
                    )}
                  </div>
